#' @title  Fit Bayesian Semi-parametric Mixture Cure Model (SMCM) via RStan
#'
#' @description
#' This is the function to fit the Bayesian SMCM model via \pkg{RStan} using normal priors for both \eqn{\bm b} and  \eqn{\bm \beta}
...
#' @param hyperpar a list containing prior parameter values for the SMCM model; among c("sigma_b", "sigma_beta", "a", "b").
...
#' \item{beta.chains}{Posterior samples for regression coefficients \eqn{\bm \beta}.}
#' \item{lambda.chains}{Posterior samples for baseline hazard rates \eqn{\bm \lambda}.}
...
#' 
#' @examples
#' # Fit SMCM RStan model to data generated from Scenario 1 in our simulation study
#' n <- 300
...
#' priorPar = list( sigma_beta  = 1000, sigma_b  = 1000, a = 0.1, b = 0.1)
#' out.smcm.rstan =  fit.SMCM.RStan(data = dat1, hyperpar = priorPar, nchains, nIter, warmup, 
#'   thin, standardize = FALSE, probs = 0 , save_loglik = 0, seed = 2025  )
...
#' 
#' @export
fit.SMCM.RStan           <- function(data, hyperpar = list(), nchains = 2, nIter = 6000, warmup = 1000, thin = 20,
  ...
  # SMCM.model           <- stan_model(file = system.file("stan", "SMCM.stan", package = "BayesSMCM")) 
  fit                  <- sampling( SMCM.model, data = data.stan.model,
