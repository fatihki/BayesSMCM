#' @title  Fit Bayesian Hierarchical Semi-parametric Mixture Cure Frailty Model (HSMCFM) with RStan
#'
#' @description
#' This function fits the Bayesian HSMCFM model using \pkg{RStan}, 
...
#' @param hyperpar a list containing prior parameter values for the HSMCFM model; among c("r1", "delta1", "r2", "delta2", "a", "b", "c", "d").
...
#' 
#' @examples
#' # Fit HSMCFM RStan model to data generated from Scenario 1 in our simulation study
#' n <- 300
...
#'   c = 0.1, d = 0.1 )
#' out.hsmcfm.rstan =  fit.HSMCFM.RStan(data = dat1, hyperpar =  priorPar, nchains, nIter, warmup, 
...
#' 
#' @export
fit.HSMCFM.RStan           <- function(data, hyperpar = list(), nchains = 2, nIter = 6000, warmup = 1000, thin = 20,
  ...
  data.stan.model       <- list(n=n, p1=p1, p2=p2, JJ=JJ, s=s, t=survObj$t, delta=survObj$di, X=survObj$X, Z=survObj$Z,
                                ...
                                theta_a0 = priorPara$c, theta_b0 = priorPara$d,
                                save_loglik = save_loglik  )
  
  # HSMCFM.model           <- stan_model(file = system.file("stan", "HSMCFM.stan", package = "BayesSMCM") ) 
  fit                    <- sampling( HSMCFM.model, data = data.stan.model,
