#' fit.MCMC.SMCFM
#' @title Fit Bayesian Semi-parametric Mixture Cure Frailty Model (SMCM) via MCMC
#'
#' @description
#' This is the main function to fit the Bayesian SMCFM model using MCMC algorithm in Kızılaslan and Vitelli (2025).
#' 
#' @name fit.MCMC.SMCFM
#'  
#' @import parallel
...
#' @param hyperpar a list containing prior parameter values for the SMCM model; among c("r1", "delta1", "r2", "delta2", "a", "b", "c", "d").
...
#' 
#' @examples
#' # Fit SMCFM model to data generated from Scenario 1 in our simulation study
#' n <- 300
...
#'   c = 0.1, d = 0.1 )
#' out.smcfm.mcmc = fit.MCMC.SMCFM ( data = dat1, hyperpar = priorPar, nchains, nIter, 
#'   warmup, thin, mcmc.parallel = "parLapply", standardize = FALSE, probs = 0, 
#'   save_loglik = 1, seed = 2025 )
...
#' 
#' @export
fit.MCMC.SMCFM           <- function(data, hyperpar = list(), nchains = 2, nIter = 6000, warmup = 1000, thin = 20, mcmc.parallel =  "parLapply",
  ...
  initial$theta.ini     <- mean(rgamma( 10, 0.1, 0.1))                  
  ...
  if( nchains > 1){
    ...
    if(mcmc.parallel == "lapply"){
      fun <- function(i) {
        ...
        return( MCMC.SMCFM(survObj, priorPara, initial, nIter, save_loglik ) )
      }
    ...
    if(mcmc.parallel == "foreach"){
      ...
      fit                  <- foreach( i=1:nchains) %dopar% {
        ...
        initial$beta.ini   <- rep(0.5*i, ncol(survObj$X) )
        initial$b.ini      <- rep(0.5*i, ncol(survObj$Z) )
        fit.mcmc           <- MCMC.SMCFM(survObj, priorPara, initial,  nIter, save_loglik )
        return(fit.mcmc)
      }
    ...
    start.mcmc.smcfm.time <- Sys.time()
    fit                   <- MCMC.SMCFM( survObj, priorPara, initial, nIter, save_loglik )
    mcmc.smcfm.time       <- as.numeric(Sys.time() - start.mcmc.smcfm.time, units = "mins")
  ...
  fit.results             <- c( b.hat = b.hat.mcmc, beta.hat =  beta.hat.mcmc, 
                                ...
                                pz.hat =  mean(pz.hat.mcmc), time = mcmc.smcfm.time )
