...
This is an R package **BayesSMCM** implementing Bayesian hierarchical modeling for the semiparametric mixture cure model (SMCM), which accommodates both the inclusion and exclusion of a frailty component [Kızılaslan F and Vitelli V, 2025](https://arxiv.org/abs/2512.08173).
...
Two covariates, $\mathbf{Z_1}$ and $\mathbf{Z_2}$ are independently generated respectively from a Bernoulli distribution with probability $0.5$, i.e. Bernoulli $(0.5)$, and from a standard normal distribution, i.e. $N(0,1)$. The covariates $\mathbf{X}$ and $\mathbf{Z}$ are assumed to be the same.
...
As an example we set $J = 1$ for the semiparametric structure. For each method, we run five MCMC chains of $7500$ iterations, discarding the first $2500$ as warm-up and applying a thinning by $10$, resulting in $2500$ posterior samples in total. See more details and full analysis in [Kızılaslan and Vitelli, 2025].
...
## References
 ...
 Kızılaslan, F., Vitelli, V. (2025). Bayesian Semiparametric Mixture Cure (Frailty) Model. [https://arxiv.org/abs/2512.08173](https://arxiv.org/abs/2512.08173).
